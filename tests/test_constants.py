...
def test_costants_file():
    ...
    assert isinstance(constants.MAIN_DOC_URL, str), (
        "В модуле `constants.py` тип переменной `MAIN_DOC_URL` " "должен быть `str`"
    )
    variables = [
        code for var, code in inspect.getmembers(constants) if not var.startswith("__")
    ]
    assert (
    ...
    assert isinstance(constants.EXPECTED_STATUS, dict), (
        "В модуле `constants.py` тип переменной `EXPECTED_STATUS` " "должен быть `dict`"
    )
