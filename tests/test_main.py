...
def test_main_file():
    ...
    assert hasattr(main, "download"), "Добавьте функцию `download` в модуль `main.py`."
    assert hasattr(main, "pep"), "Добавьте функцию `pep` в модуль `main.py`."
    assert hasattr(main, "MODE_TO_FUNCTION"), (
        "Добавьте словарь `MODE_TO_FUNCTION` с перечнем режимов " "работы парсера."
    )
...
def test_latest_versions(mock_session):
    ...
    assert got == answer, (
        "Функция `latest_versions` должна возвращать " f"объект вида ```{answer}```"
    )
...
def test_download(monkeypatch, tmp_path, mock_session):
    ...
    output_files = [f for f in mock_base_dir.glob("**/*") if str(f).endswith(".zip")]
...
def test_mode_to_function():
    ...
    for name_func, func in got.items():
        ...
        assert name_func in ["whats-new", "latest-versions", "download", "pep"], (
        ...
        assert func.__name__ in ["whats_new", "latest_versions", "download", "pep"], (
            "В модуле `main.py` в объекте `MODE_TO_FUNCTION` " f"нет значения {func}"
        )
